...
#define USRP_DEMODULATOR_INCLUDED 1
...
class RX_buffer_demodulator{
    public:
            ...
            switch(last_w_type){
                ...
                case CHIRP: //valid only for single chirp
                    ...
                    if(decimator_active){
                        ...
                        profile = make_hamming_window<float2>(ppt,ppt/10,true);
        ...
        int process(float2** __restrict__ in, float2** __restrict__ out){ return (this->*process_ptr)(in,out); }
    ...
    private:
        ...
        int process_chirp(float2** __restrict__ input_buffer, float2** __restrict__ output_buffer){
            ...
            chirp_demodulator<<<1024,32,0,internal_stream>>>(input,output+spare_size,parameters->buffer_len,last_index,d_parameter);
            
            //update bookeeping index
            last_index = (last_index + parameters->buffer_len) % (h_parameter.num_steps * h_parameter.length);
            
            //print_debug("last_index",last_index);
/*          
            move_buffer<<<1024,64,0,internal_stream>>>(
                        input,  //from
                        output,  //to
                        parameters->buffer_len, //size
                        0,spare_size); // destination offset
*/
            ...
            if(decimator_active){
                ...
                //decimate the demodulated samples
                //decimate_vna(output,decim_output,parameters->decim,ppt,side,   valid_size,   internal_stream,  vna_helper->total_len);
                //print_debug("ppt ",ppt);
                //print_debug("valid_size ",valid_size);
                ...
                if(spare_size > 0){
                    move_buffer<<<1024,64,0,internal_stream>>>(
                        ...
                        vna_helper->spare_begin,0); // destination offset
                }
        ...
        int process_pfb(float2** __restrict__ input_buffer, float2** __restrict__ output_buffer){
            ...
            if(decimator_active){
                ...
                tone_select<<<1024,64,0,internal_stream>>>(output, reduced_output, d_params, std::floor(buf_setting->current_batch/(float)parameters->decim));
                ...
                //move the selected tones in an other buffer excluding exceeding samples
                tone_select<<<1024,64,0,internal_stream>>>(output, reduced_output, d_params, buf_setting->current_batch);
